...
def _mmss_to_seconds(mmss: str) -> float:
    parts = mmss.split(":")
    if len(parts) != 2:
        raise ValueError(f"Invalid time format '{mmss}': expected MM:SS")
    try:
        return int(parts[0]) * 60 + float(parts[1])
    except ValueError:
        raise ValueError(f"Invalid time format '{mmss}': minutes and seconds must be numeric")


def _seconds_to_srt(total_seconds: float) -> str:
